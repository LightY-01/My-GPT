...
> **Note on Generation Cutoffs:** You may notice the model occasionally ends abruptly mid-word or mid-sentence (e.g., `the lords o`). This is expected behavior. Because this is a character-level model running on a strict token budget (`max_new_tokens`), the generation loop terminates exactly when the character count is reached, regardless of punctuation.
...
## Byte-Pair Encoding (BPE) Integration

While the core repository focuses on the character-level implementation, this architecture has been successfully scaled to support token-level processing. 

The complete BPE migration pipeline is fully documented and contained within the [bpe-integration](./bpe-integration) subdirectory.
