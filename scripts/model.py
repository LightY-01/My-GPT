...
# Used torch.manual_seed(0) initially in forward pass also
# So Dropout was generating the SAME mask every forward pass,
# permanently killing the same 20% of neurons every call.
# Removed torch.manual_seed(0) — it was a big bug!

# Added a KVCache class to store key and value states from previous forward passes
# This is used to optimize the forward pass by avoiding redundant computations
class KVCache:
    def __init__(self, context_length: int): # Added context_length
        # (batch, num_kv_heads, seq_len, head_dim)
        self.cache_k: Optional[torch.Tensor] = None  
        self.cache_v: Optional[torch.Tensor] = None
        # Stored context_length for positional embedding logic
    ...
    def update(self, new_k: torch.Tensor, new_v: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Truncate the cache to context_length
        # This keeps the most recent context_length tokens
